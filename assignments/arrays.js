...
let carModels = inventory.slice();
carModels.sort(function(a , b) {
    let car_modelA = a.car_model.toLowerCase();
    let car_modelB = b.car_model.toLowerCase();
    if (car_modelA < car_modelB) {
        return -1;
    } if (car_modelB < car_modelA) {
        return 1;
    }
})
...
// }
inventory.forEach(item => carYears.push(item.car_year))
...
inventory.forEach(item => {
    if (item.car_make === "Audi" || item.car_make === "BMW") {
        BMWAndAudi.push(item);
    }
})
//JSON.stringify(BMWAndAudi, ["id", "brand", "model", "year"]);
console.log(JSON.stringify(BMWAndAudi, ["id", "car_make", "car_model", "car_year"], "/t"));
